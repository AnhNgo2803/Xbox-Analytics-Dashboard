...
def h_bar():
    ...
    # Loop through the container containing the bars

    # ax_gen_tab.containers[0] is the BarContainer, contain a bar (patches)
    for container in ax_gen_tab.containers:
        # container.datavalues ​​contains an array of values ​​(e.g. [455839, 412763, ...])
        # container.patches contains a list of Rectangle objects (bars)
        
        # Combine the value and the corresponding bar object
        for value, bar in zip(container.datavalues, container.patches):
            y_pos = bar.get_y() + bar.get_height() / 2
            
            # Take the maximum limit of the x-axis to calculate the position relatively
            _, xmax = ax_gen_tab.get_xlim() # remove minimum use "_," take max (_,_) 

            # If value of bar < 20% max x axis, pull label out of bar
            if value < (xmax * 0.20):
                ...
                # if value is large enough put inside bar
    ...
    # Extend the x-axis bounds to ensure outer labels are not cut off
...
[My project summary on LinkedIn](https://www.linkedin.com/in/duyanhngo03/details/projects/?profileUrn=urn%3Ali%3Afsd_profile%3AACoAAEPiiK8BJoFIdOLPSlFkTRoHso99100cg-c)
:::
